...
| 3.5 | Clases especiales: coreografías puntuales fuera de la parrilla, con video y precio propio | PRD-0018 (borrador) |
...
| Fecha | Cambio |
|---|---|
| 9 sep 2026 | **Migración de Pau aplicada en staging y en producción**, con aprobación en cada caso y antes-después por SQL: 19 clases viejas fuera, 21 nuevas dentro con la Girly del 08/09, sin reservas tocadas. **Cron de generación arreglado**: `CRON_SECRET` cargada en Vercel junto a `CRON_SECRETO`; una invocación disparada por Vercel respondió 200. **PRD-0018 §8 resuelto: las clases especiales se compran aparte**, no con créditos, porque tienen precios distintos |
| 8 sep 2026 | **Cambio de horario de Pau** por un problema de salas: Girly pasa de lunes 20:00 a **martes 20:00** y Reggaeton Femme de miércoles 20:00 a **jueves 19:30**, ambas en Diaguitas. Migración `20260908150000_horarios_pau_martes_y_jueves.sql` escrita ese día, aplicada el 9. Como `clases` copia el horario al generarse, la migración también corta las 18 clases futuras ya materializadas: borra las que nadie reservó y cancela con motivo las que tengan reserva, para que el trigger devuelva el crédito. El cambio opera desde el lunes 07/09: la migración borra también la clase de ese lunes, que no se dictó, y registra la Girly del martes 08/09 en el horario nuevo. Se detectó de paso que **el cron de generación no crea clases desde el 31/08** (la última materializada es del 09/11): Vercel solo manda la cabecera `Authorization` cuando la variable se llama exactamente `CRON_SECRET`, y el proyecto la llama `CRON_SECRETO`, así que cada invocación del cron recibe 401. Diagnosticado, sin arreglar todavía |
| 8 sep 2026 | Se crea **PRD-0018 — Clases especiales** (borrador) con su plan: clases puntuales de coreografía, fuera de la parrilla, con video y precio propio. Deja **marcada sin resolver** la pregunta de si se pagan con créditos o son compra aparte (§8), y advierte que "clase suelta" ya es el nombre del plan de 1 clase |
| 8 sep 2026 | **PRD-0010 parte 1: el tablero de owner**, en `prd-0010-metricas`. Créditos, venta, demanda, alumnas y operación, cada indicador con su comparación contra el mes anterior y su absoluto junto al porcentaje. Lo primero de la página es la **brecha entre lo vendido y lo entregado**, que es lo que la caja esconde en un modelo prepagado. Como no hay compras reales todavía, el problema no era calcular sino **saber si el cálculo está bien cuando todo da cero**: se creó un proyecto Supabase de staging, se sembró un escenario de 5 alumnas, 8 compras y 5 clases con los 22 valores esperados **calculados a mano y escritos en el PRD antes de programar**, y se contrastaron contra el SQL y contra la página renderizada. Para no confiar en un 22 de 22 a la primera se mutó la base dentro de transacciones revertidas: el arnés detecta las mutaciones y la franja de conciliación caza los descuadres. 🔴 **Dos defectos del SQL aparecieron recién al escribir la interfaz, y los dos son el mismo error**: dar por supuesto que el período contesta todo. Deducía el mes anterior de su largo —contra un agosto al que le faltaba un día— y no traía con qué escribir "última compra: nunca". El escenario no los agarró porque usaba una ventana de 30 días, donde las dos definiciones coinciden; se ancló al mes calendario. Y **abrir la página encontró cinco defectos más** que ni el compilador ni el SQL podían ver, del tipo `$-55.500` y "1 clases dictadas". Se recortan tres métricas **con el motivo en pantalla en vez de un cero**: no-shows (nadie registra asistencia), caja neta y margen (son la parte 2) e ingresos de Teens (PRD-0011 no existe). Primeros tests del proyecto: 37 con `node --test`, sin dependencias nuevas. ⚠️ **Migración escrita y sin aplicar a producción**; solo está en staging |
| 8 sep 2026 | **Definido el variable de las profesoras**, que estaba pendiente desde agosto: **$250 por crédito consumido** en packs y **$1.000 por alumna inscrita al mes** en Teens, que va por inscripción porque no reserva ni consume créditos. Son dos bases de cálculo distintas y quedan contrastadas en `CONTEXT.md` §5.b. Se recalcula la economía unitaria: **el piso sube en tres de los seis casos** —con packs de 8 en Los Leones hacen falta 7 alumnas y no 6— y el neto con sala llena baja a $113.500. El variable **no baja el techo, aplana la pendiente**: cada alumna adicional deja $250 menos, pero un horario vacío sigue costando lo mismo. Se define además cómo se paga una clase que no se dictó: nada si la causa es de la profesora, la base si es de XO. Un reemplazo **no necesita rol nuevo** —`clases.profesora_id` ya puede diferir del horario y se le paga a quien dictó— así que falta la operación, no el modelo |
