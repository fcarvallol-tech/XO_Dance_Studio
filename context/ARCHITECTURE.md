...
| ~~Flujo de cancelación sin probar con datos~~ | ✅ **Saldada el 08/09/2026.** El escenario de PRD-0010 §11.4 lo ejercita en staging: `reservar()`, `cancelar_reserva()` dentro y fuera de ventana, y el trigger de clase cancelada disparado con un `update`, que es como se cancela hoy. El crédito vuelve al lote original en los dos casos que corresponde |
| ~~`reservas` sin prueba con datos~~ | ✅ **Saldada el 08/09/2026** en staging, con nueve reservas sembradas. Queda pendiente repetirlo contra producción con la primera reserva real |
...
| **El webhook de revalidación nunca se creó** | Confirmado por Felipe el 26/09/2026: el Database Webhook de Supabase que llama a `/api/revalidar` **no existe**. La ruta sí está y responde 401 (pide su secreto), pero nadie la llama. **Consecuencia:** un cambio hecho en el **Table Editor** —un precio, una bio, una sede, activar un curso— tarda **hasta una hora** en verse, que es el `revalidate = 3600` de las páginas. No es una falla silenciosa nueva: PRD-0015 §11 lo dejó escrito como el caso "si nadie lo configuró". Lo que **sí** se ve al instante es lo que se cambia desde los portales —publicar una clase especial, por ejemplo—, porque esas acciones llaman a `revalidatePath` directo. Crearlo es configuración de panel: Database → Webhooks, sobre `cursos`, `profesoras`, `sedes`, `horarios`, `planes` y `clases`, apuntando a `/api/revalidar` con la cabecera `x-revalidar-secreto` |
...
| Sin tests | ✅ **Parcialmente saldada el 07/09/2026:** `lib/dominio/` tiene 37 tests con `node --test`, sin dependencias nuevas — Node 24 ejecuta TypeScript sin transpilar. Cubre las métricas y los períodos. **Falta lo que mueve créditos de verdad** (`reservar`, `cancelar_reserva`, `acreditar_compra`), que vive en SQL y hoy solo se prueba por el escenario sembrado. `lib/lead.ts` sigue sin tests |
| Nadie registra asistencia | `reservas.estado` acepta `no_asistio` y **ninguna función lo escribe**. La métrica de no-shows no existe por eso (PRD-0010 §6.1). No bloquea la liquidación de profesoras: su variable va por crédito consumido, no por alumna presente. Necesita PRD propio |
| Nada emite el movimiento de expiración | `movimientos_credito.tipo` acepta `expiracion` y ningún proceso lo escribe: un lote vencido queda con `cantidad_disponible > 0` para siempre. La conciliación del tablero cuadra igual porque ni el libro ni los lotes lo descuentan, pero el día que exista ese proceso tiene que **escribir el asiento y bajar el disponible**, o la conciliación se rompe |
| `leads` no se puede unir con la cuenta | Falta `leads.perfil_id`, que `ARCHITECTURE.md` §6 pide desde agosto. Por eso el embudo del tablero empieza en "cuenta creada" y no en el lead (PRD-0010 §6.2) |
