...
export default async function Compras() {
  ...
  const enJuego = pendientes.reduce((total, compra) => total + compra.monto, 0);
    ...
    <>
      ...
      {pendientes.length > 0 ? (
        <p className="mb-8 text-xo-gris">
          <strong className="text-xo-negro">{pendientes.length}</strong>{" "}
          {pendientes.length === 1 ? "esperando" : "esperando"} ·{" "}
          {clp(enJuego)} por confirmar
        </p>
      ) : null}

      <BandejaCompras pendientes={pendientes} />
