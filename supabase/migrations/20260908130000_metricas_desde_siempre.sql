-- ---------------------------------------------------------------------------
-- Un cero tiene que poder decir de qué cero se trata — PRD-0010 §9.2
-- ---------------------------------------------------------------------------
--
-- QUÉ PROBLEMA RESUELVE
--
-- El PRD promete que todo cero venga "con su denominador y con la fecha del
-- último dato": no un `0`, sino "0 compras en septiembre · última compra
-- registrada: nunca". Con solo las cifras del período eso no se puede escribir,
-- porque un mes en cero y una base vacía se ven exactamente igual — que es el
-- problema que originó este PRD.
--
-- Se agrega `desde_siempre`: los totales sin filtro de período y la fecha del
-- último movimiento, la última compra, la última reserva y la última clase
-- dictada.
--
-- ⚠️ Es la segunda corrección a esta función en dos días, y las dos son la
-- misma clase de error: **dar por supuesto que el período contesta todo**. La
-- primera dedujo el período anterior de su largo; esta olvidó que hay preguntas
-- que no son de ningún período. Vale anotarlo para la próxima.
-- ---------------------------------------------------------------------------
...
comment on function public.metricas_resumen(timestamptz, timestamptz, timestamptz, timestamptz) is
  'Créditos, venta, alumnas y operación de un período y del anterior, más los totales desde siempre para que un cero se pueda distinguir de un vacío. Agrega y agrupa; no divide. Solo owner.';
